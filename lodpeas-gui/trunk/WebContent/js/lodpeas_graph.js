	...
	var fd = new $jit.ForceDirected(
			{
				...
				Events : {
					...
					onClick : function(node) {
						...
						if (node.nodeFrom) {
							...
							// connections.
							var html = "<h4><b>comparing:</b><ul><li><span class=\"node-label\" title=\""+node.nodeTo.id+"\">" + node.nodeTo.name + "</span></li>"
									+ "<li><span class=\"node-label\" title=\""+node.nodeFrom.id+"\">" + node.nodeFrom.name + "</span></li></ul></h4><div class=\"score\"><b>score: </b><span class=\"score-val\">" + node.data.score
									+ "</span></div>";
                ...
                for (i = 0; i < node.data.common.length; i++) {
                  if (node.data.common[i].dir === "o") {
                    outward = true;
                  } else if(node.data.common[i].dir === "s") {
                    inward = true;
                  }
                }
              
                if(outward){
                  html += "<b> both have values: </b><div class=\"common-values\"><ul>";
  								for (i = 0; i < node.data.common.length; i++) {
                    if (node.data.common[i].dir === "o") {
    									var plabel = node.data.common[i].plabel;
    									var vlabel = node.data.common[i].vlabel;
    
    									if (typeof (plabel) == 'undefined' || plabel == null || plabel.replace(/^\s+|\s+$/g, "") === "") {
    										plabel = "<i>unlabelled</i>";
    									}
    									if (typeof (vlabel) == 'undefined' || vlabel == null || vlabel.replace(/^\s+|\s+$/g, "") === "") {
    										vlabel = node.data.common[i].vnode;
    									}
    
    									plabel = "<span class=\"common-pred\" title=\""+node.data.common[i].pnode+"\">" + plabel + "</span>";
    
    									
    									html += "<li><span class=\"outward\">" + plabel + " :<ul><li><span title=\""+node.data.common[i].vnode+"\"> " + vlabel + " </span><span class=\"shared_by\">[shared by "
    												+ node.data.common[i].n + "]</span></span></li></ul></li>";
    								}
    							}  
                ...
                if(inward){
                  html += "<b> both are values for: </b><div class=\"common-values-for\"><ul>";
                  ...
                  for (i = 0; i < node.data.common.length; i++) {
                    if (node.data.common[i].dir === "s") {
    									...
    									plabel = "<span class=\"common-pred\" title=\""+node.data.common[i].pnode+"\">" + plabel + "</span>";
  										...
  										html += "<li><span class=\"inward\">" + plabel + " :<ul><li><span title=\""+node.data.common[i].vnode+"\"> " + vlabel + " <span class=\"shared_by\">[shared by "
  												+ node.data.common[i].n + "]</span></span></li></ul></li>";
    								}
    							}  
                  html += "</ul></div>";
